...
class Wappie(Agent):
    def __init__(self, unique_id: int, model: Model, pos, prior_beliefs: List[float]) -> None:
    ...
    def assimilation(self):
        ...
        beliefs_copy += self.model.mu * np.subtract(beliefs_copy, interacting_neighbor_beliefs_copy)
        interacting_neighbor_beliefs_copy += self.model.mu * np.subtract(interacting_neighbor_beliefs_copy, beliefs_copy)
    ...
    def contrast(self):
        ...
        new_prog_cons_1 = self.normalisation(prog_cons_1 - self.model.lambd * (prog_cons_2 - prog_cons_1))
        new_prog_cons_2 = self.normalisation(prog_cons_2 - self.model.lambd * (prog_cons_1- prog_cons_2))

        new_left_right_1 = self.normalisation(left_right_1 - self.model.lambd * (left_right_2 - left_right_1))
        new_left_right_2 = self.normalisation(left_right_2 - self.model.lambd * (left_right_1 - left_right_2))

        self.belief = np.array([new_prog_cons_1, new_left_right_1])
        self.interacting_neighbor.beliefs = np.array([new_prog_cons_2, new_left_right_2])

    def step(self):
        # get neighbors from grid and network
        neighbors_grid = self.model.grid.get_neighbors(self.pos, moore=True)
        neighbors_network = self.model.network.get_neighbors(unique_id, moore=True)
...
class Political_spectrum(Model):
    def __init__(self, width: int, height: int, lambd: float, mu: float, 
                 d1: float, d2: float, mu_norm: float, sigma_norm: float) -> None:
...
if __name__ == "__main__":
    ...
    # initialise model
    model = Political_spectrum(width=3, height=3, lambd=0.05, mu=0.20, d1=0.35, d2=1.5, mu_norm=0.5, sigma_norm=0.45)
